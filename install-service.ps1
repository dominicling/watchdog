...
<#
.SYNOPSIS
  Install WatchdogService via NSSM.
...
$ScriptPath   = Join-Path $WatchdogDir 'watchdog.js'
...
# Service parameters — embed script path directly in AppParameters
# (GPO resets machine ExecutionPolicy, but this is pure Node so irrelevant here)
& $NssmExe set $ServiceName AppParameters      "`"$ScriptPath`""
& $NssmExe set $ServiceName AppDirectory       $WatchdogDir
& $NssmExe set $ServiceName DisplayName        'Watchdog Security Monitor'
& $NssmExe set $ServiceName Description        'Monitors protected paths for bash.exe access; kills process and sends alert.'
...
# Restart policy: restart on crash after 5s, up to 3 times per hour
